...
class MovieRepository:
    ...
    async def upsert_movie(self,*,provider: str,provider_id: str,title: str,
    overview: str | None,
    ...
    genres: list[tuple[str, str]],) -> Movie:
        movie = await self.get_by_provider_id(
            provider,
            provider_id,
        )
    ...
    async def list_popular(self, *, limit: int = 30) -> list[Movie]:
        stmt = select(Movie).options(selectinload(Movie.genres).selectinload(MovieGenre.genre)).order_by(Movie.popularity.desc().nullslast()).limit(limit)
        return list(await self.session.scalars(stmt))

    async def list_candidates(self, *, exclude: set[UUID], preferred_genre_ids: list[UUID], limit: int) -> list[Movie]:
        stmt = select(Movie).options(selectinload(Movie.genres).selectinload(MovieGenre.genre)).where(Movie.id.not_in(exclude))
